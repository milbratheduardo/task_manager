...
const MyTasks = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (response.data) {
        const assignedTasks = (response.data.tasks || []).filter((task: Task) =>
          task.assignedTo.some(member => member._id === user?._id)
        );
  ...
  return (
    <DashboardLayout activeMenu="Minhas Tarefas">
      <div className="mb-6">
        <h2 className="text-2xl font-semibold mb-2 text-gray-800">Minhas Tarefas</h2>
